...
### 1. Input cost pass-through under sustained inflation — Supplier Power × Buyer Power interaction

This channel sits at the *intersection* of the supplier-power and buyer-power forces (the two strongest in the industry archetype). The §7.4 archetype's `supplier_power: moderate` rating implies that under sustained input-cost inflation, mining-customer push-back blocks full pass-through. DNL's long-term US gas contracts shield ~70% of US gas exposure through 2028, but Australian gas (no long-term contract position) and ammonia spot exposure (where contract-uncovered) flow through.
...
### 2. Geographic / bloc-alignment exposure — New Entrants / Substitutes (regulatory)
...
### 3. Terminal-state exposure to coal-customer attrition — Buyer Power (customer-base trajectory)
...
## Step 3 company-position decomposition (Five Forces spine, methodology §3.3)

Per the framework's company-position discipline, DNL's deviation from the industry baseline is organised force-by-force:

| Force | Industry rating | DNL exposure | Mechanism | Δ |
|---|---|---|---|---:|
| Buyer power | HIGH (binding) | at industry average | Same mining-major customer mix as Orica | 0 |
| Supplier power | MODERATE | LOWER (favourable) | Long-term US gas contracts through 2028 | Margin +200bps now → 0 by FY31 (gas roll-off overlay) |
| New entrants | LOW | LOWER (favourable in EM) | DNL itself is new entrant via DNEL ramp | Growth +15bps |
| Substitutes | LOW | at industry average | Intra-industry tech sub affects all players | 0 |
| Rivalry | MODERATE | HIGHER (unfavourable) | DNL #2 vs Orica #1; scale + tech lead | Growth -30bps |
| Rivalry (product mix) | (sub) | unfavourable | More bulk-ANFO; less electronic detonators | Growth -10bps |
| **Net growth offset** | | | | **−25bps** |

This is the spine of the company-position analysis. Each offset is articulable, force-anchored, and avoids double-counting with overlays elsewhere in the model. Worked example in `data/companies/dnl.md` (full Five Forces structure) and `analyses/dnl/valuations/dnl_muddle_through_valuation_v4_5forces.xlsx` (numerical implementation).
...
1. **Per-entity functional currency.** Per IAS 21 / AASB 121: parent AUD-functional, Americas subsidiary USD-functional, Australian subsidiary AUD-functional. Most published Australian DCFs collapse to AUD because it's the reporting currency, losing the USD-dominant US economics in FX translation noise.
...
3. **Gas-contract roll-off as a structural-headwind overlay.** Per methodology §3.2.1, the 2028-2030 contract erosion is built into the margin path explicitly (-50 / -100 / -150 bps cumulative Y3 / Y4 / Y5) rather than hand-waved to terminal state. Consensus DCFs typically miss this.
4. **Single-WACC discipline.** Per methodology §3.5, WACC is held constant across scenarios. Scenario risk is captured in cash flows, not in discount-rate gymnastics. Consensus DCFs often vary WACC by scenario, double-counting risk.
5. **Tax-rate discipline.** Per methodology §3.6, effective rate (per management guidance) glides to blended statutory (computed from jurisdictional weights × statutory rates) over the explicit horizon. Single rate consistency across operating tax / WACC debt-tax-shield / Hamada re-levering. Consensus DCFs typically hold effective rate flat into perpetuity.
6. **Five Forces spine for company position.** Per methodology §3.3, Step 3 traverses every force with named mechanism + quantified delta + where-captured. Replaces hand-wavy "company offsets" with articulable, force-anchored differentials.
7. **Coal-customer-attrition reflected in terminal state.** Per §9.7 item 9 (climate-trajectory analogue) and §10.6 defended-exception discipline. Consensus DCFs typically use a static terminal ROIC.
8. **Fragmentation as a modelled scenario, not just a risk-register item.** Per the §3 workshop output, Fragmentation has unique DNL exposure that conventional analysis typically captures only as a sensitivity, not as a modelled scenario.
...
- Company positioning: `data/companies/dnl.md` (Five Forces spine refactor 9 June 2026).
