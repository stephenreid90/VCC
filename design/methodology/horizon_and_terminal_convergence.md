...
Live five-year levels against the proposal.

> **Assumption set `paper_s10`** — SUPERSEDED by `second_sitting` (§13a).
> Horizon 10 years; growth fades to g across the final 5; explicit capex converges to
> 7.3% by Y5; gas roll-off −2.0pp on the §8 phasing; Disorderly capex arc per §9;
> terminal capex taken from the final explicit year; invested capital 3,681.1m (D-44).
> Declared in `design/methodology/horizon_variant_sets.yaml`, generated by
> `scripts/size_horizon_variants.py`, asserted by `tests/dcf/test_horizon_variant_sets.py`.
> The levels below are the published ones; the generated levels agree to 0.01%.
...
> **Assumption set `paper_s13_ruled`** — SUPERSEDED by `second_sitting` (§13a).
> Horizon 10 years; growth fades to g across the final 5; explicit capex **held on the
> live path**, which converges to 7.0%; gas roll-off −1.5pp on the §8 phasing; Disorderly
> capex arc per §9; terminal capex set so the capital base compounds at g, with Disorderly's
> persistent +1.0pp licence-to-operate shift added on top of a base grown ex-arc;
> invested capital 3,681.1m (D-44). Generated levels agree with the published ones to 0.22%.
...
## 13a. The current table, and what the two published tables actually differed by

The two tables above were struck on different assumptions and neither said so, which is
how they came to be quoted against each other as though the difference were noise. It is
not noise and it is not the boundary conventions: **the whole of it is the explicit capex
path.** `paper_s13_ruled` holds the live path, which converges to 7.0%; `second_sitting`
applies D-38 and converges it to 7.3% by Y5. Nothing else moves between them.

> **Assumption set `second_sitting`** — CURRENT.
> Horizon 10 years; growth fades to g across the final 5; explicit capex converges to
> 7.3% by Y5 (D-38); gas roll-off −1.5pp on the §8 phasing (D-40); Disorderly capex arc
> per §9 (D-41); terminal capex set so the capital base compounds at g, the arc included
> in that base and no separate carbon premium; invested capital 3,681.1m (D-44).

| Scenario | `paper_s13_ruled` | **Current** | Change | Terminal share | Terminal capex |
|---|---|---|---|---|---|
| Orderly Convergence | 3.2343 | **3.1793** | −1.7% | 53.6% | 8.78% |
| Muddle Through | 2.7451 | **2.6956** | −1.8% | 51.3% | 8.77% |
| AI Productivity Lag | 2.7021 | **2.6558** | −1.7% | 50.1% | 8.69% |
| Fragmentation | 1.9112 | **1.8629** | −2.5% | 49.7% | 8.62% |
| Disorderly Climate | 1.4383 | **1.5294** | +6.3% | 57.1% | 8.56% |
| Stagflation Persists | 0.6302 | **0.5841** | −7.3% | 44.7% | 8.69% |

Disorderly Climate moves the other way, and for a reason worth recording rather than
averaging away. The two sets do not merely differ in the capex path there; they take
different views of what the arc *is*. `paper_s13_ruled` treats the persistent +1.0pp as a
licence-to-operate cost — money spent to keep operating, not capital that earns a return —
so it grows the base excluding the arc and adds the 1.0pp on top, giving a terminal capex
of 9.22%. `second_sitting` treats the arc as ordinary capital, grows the base including
it, and lands at 8.56%. That is a live methodology question, not a reconciliation
residual, and §9's own reasoning ("a licence-to-operate cost is permanent by
construction") argues for the first reading. It is put with item 11 rather than settled
here, because both readings are only worth striking once the capital-intensity question
below is answered.

## 13b. Item 11 — the three positions, sized

Under the current set, invested capital falls from 108.3% of revenue at the valuation date
to 67.5–85.8% at Y10, because capex converging to D&A means DNL adds no net fixed capital
across ten years while revenue compounds about 50%. That thinning, not a moat, is what
produces the terminal ROIC. Three positions were put and none taken:

| Scenario | Ruled build | Hold intensity flat | Volume plus asset inflation |
|---|---|---|---|
| Orderly Convergence | 3.1793 | 1.9804 | 1.8671 |
| **Muddle Through** | **2.6956** | **1.7676** | **1.8387** |
| AI Productivity Lag | 2.6558 | 1.8630 | 1.9801 |
| Fragmentation | 1.8629 | 0.9497 | 1.3305 |
| Disorderly Climate | 1.5294 | 0.9485 | 1.5335 |
| Stagflation Persists | 0.5841 | −0.1998 | 0.3308 |

The evidence question underneath it is answered rather than open. The split of DNL's
nominal growth into volume and price is already computed in the chain — B25 industry
volume growth and B29 industry pricing growth, per scenario. Muddle Through is 3.28%
volume against 2.85% pricing. So the hope that most of the growth is price, and capacity
therefore need not follow, does not survive: volume is over half of it, and replacement
assets inflate as well. Both positions that make capital follow the business land in the
1.77–1.84 range on the central case. The ruled build, at 2.6956, is the outlier — it is
roughly a third of the company above either.

Disorderly Climate is the exception, and the reason is mechanical: its capex arc, not the
convergence rule, sets its path, so the three positions collapse together there. Item 11
should be argued on the other five.

**The volume-plus-inflation column is the fading construction**: capital tracks the volume
half of each year's revenue growth, inflated at asset inflation, so the capital call slows
as the fade slows revenue. The 25 August handover quoted this position with the chain rate
held flat for all ten years (Muddle Through 1.9551); that run was not committed and does
not reproduce. Held flat, the same position gives 1.6175 on the central case — lower, not
higher, because a constant capital call outruns a fading revenue line in the back half.
Both readings are defensible and both are below the ruled build, which is what the
decision turns on.
