...
### 4.6 Lease-normalisation convention (AASB 16 / IFRS 16)

**Added v0.8 (12 July 2026).** Leases touch the per-share number in three places — the operating build (they sit inside EBIT / EBITDA / FCFF), the net-debt / equity bridge, and the multiples cross-check — and a single convention must hold across all three. The failure mode is a *mixed basis*: the numerator computed in the leases-are-debt world while the bridge is computed in the leases-are-operating world (or vice versa). This section codifies the convention.

**4.6.1 Default convention — Approach A (leases as debt).** Keep the company's reported post-AASB 16 earnings — in which rent has already been replaced by right-of-use depreciation plus lease interest, so lease cost has left EBIT and EBITDA — and add the lease liability into net debt so both sides of the bridge sit in the same world. This is preferred over de-capitalising back to an operating-rent basis (Approach B) because it uses reported numbers with the least estimation and scales to lease-heavy names. Approach B is retained only as a fallback where a company's reported accounts are pre-AASB 16 or otherwise incompatible.

**4.6.2 Subject-consistency rule (the invariant).** Whatever tenure assumption sets the lease liability in net debt must be the same assumption behind the earnings figure. On reported numbers this is automatic — earnings and the liability come from the same accounts under the same lease term. The trap is normalising one side without the other (e.g. stripping the liability from net debt while leaving post-AASB 16 earnings in the numerator, or re-estimating the liability on a longer tenure without re-cutting the rent stream). The translator must never produce a bridge in which the lease liability and the earnings basis disagree.

**4.6.3 Materiality gate — two ratios, shown on screen.** For each company compute two ratios: (a) lease liability ÷ enterprise value (or ÷ lease-inclusive net debt); and (b) annual lease cost (RoU depreciation + lease interest ≈ cash lease payment) ÷ EBITDA. A threshold of ~10–15% on *either* ratio flips a company from "apply the convention, show the calc, move on" to **lease-sensitive**. Both ratios *and* the threshold must be surfaced in the deliverable — the verdict is shown, not asserted. Below the threshold, Approach A alone suffices and no lease-neutral peer basis is required.

**4.6.4 Why cross-company is a multiples problem, not a DCF problem.** In the subject's own DCF, earnings and the lease liability are drawn from the same accounts under the same tenure assumption, so a longer or shorter lease term moves enterprise value and the liability *together* and they offset at the equity line — the DCF discounts the real rent stream, so the accounting carve-up between "operating cost" and "financing liability" nets out. A multiple cannot net it out: EBITDA is tenure-invariant, but the lease liability that adjusts EV to equity (or that a peer capitalises) is tenure- and standard-dependent, so the distortion passes straight through to the comparison. Lease normalisation is therefore a discipline for the **comparables cross-check**, not for the intrinsic build.

**4.6.5 Aligning peers — house re-capitalisation to a lease-neutral EV/EBITDAR basis.** Do not consume reported lease liabilities cross-company (they embed each company's own tenure and standard choices). Instead rebuild each peer's lease capitalisation from the underlying rent commitment on one house rule with three fixed knobs, applied identically to the subject and every peer:

  1. **Capitalisation method** — a uniform multiple (e.g. rent × 8) or a uniform rate-and-term (discount the committed rent at one rate over one assumed term). The multiple is the default for its simplicity and transparency.
  2. **Tenure definition** — e.g. committed-to-first-break, or committed plus one standard renewal — applied to every name.
  3. **Rent definition** — whether variable / turnover rent is included, applied to every name.

The rebuilt capitalisation replaces whatever lease liability sits in reported net debt, and the rent add-back lifts EBITDA to **EBITDAR** (earnings before rent). The resulting **EV/EBITDAR** — adjusted EV over EBITDAR, both on the house rule — is the lease-neutral peer basis. This mirrors the ratings-agency house treatment (Moody's / S&P re-capitalise reported leases onto a single internal rule rather than trusting each issuer's disclosed figure). In the interface it is surfaced as a lease-neutral basis on the multiples toggle, **gated to lease-heavy archetypes** (the three current test companies — DNL, WBC, CSL — never see it).

**4.6.6 AASB 16 / IFRS 16 versus US GAAP (ASC 842).** The balance sheets broadly align — both capitalise a right-of-use asset and a lease liability. The income statement and cash flow do **not**: AASB 16 / IFRS 16 is single-model (all leases become depreciation plus interest, so all lease cost leaves EBITDA and operating cash flow), whereas ASC 842 keeps operating leases as a single straight-line operating expense that stays *inside* EBIT / EBITDA and operating cash flow. Identical lease economics therefore report different EBITDA and OCF across the two standards. The uniform EBITDAR basis in §4.6.5 strips this divergence out as well, because it adds rent back regardless of where the standard put it — which is why the peer basis carries an accounting-standard flag per company (§4.6.8).

**4.6.7 DNL worked example (lease-light).** DNL's FY25 accounts carry a AUD 212m lease liability as a line separate from short- and long-term borrowings. The liability is ~2.6% of enterprise value and the annual lease cost is ~6–7% of EBITDA — both below the §4.6.3 threshold, so DNL is **lease-light**. The company was nonetheless in a latent mixed state: reported post-AASB 16 earnings in the numerator, but net debt defined to *exclude* the lease liability. Folding the 212 into net debt (Approach A) is a ~AUD 0.11 / ~3% one-directional correction to the per-share number. This is taken not on a materiality judgment but as a **consistency** fix under §4.6.2 — the numerator was already in the leases-are-debt world, so the bridge must be too. No lease-neutral peer basis is invoked for DNL; the correction re-anchored the six scenarios (Muddle Through 3.59 → 3.48), leaving the downside/upside asymmetry ratio unchanged.

**4.6.8 Data contract (flag to the data workstream).** Re-capitalising peers on our own house rule needs four fields per company that the reported liability alone does not give us: (a) whether reported total debt already includes AASB 16 / IFRS 16 lease liabilities; (b) the annual rent / lease-cost line (RoU depreciation + lease interest); (c) the undiscounted lease-maturity table, to re-discount or re-capitalise on the house rule; and (d) an accounting-standard flag (AASB 16 / IFRS 16 versus US GAAP ASC 842) per company. These are carried in the company-position structured data as a `lease_contract` block (mock until the EODHD feed returns; see the `_leaseContract` stub on the DNL config).

**Discipline test (auditor-facing):** for any company, the workbook lint should confirm that if a lease liability appears on the balance sheet it is wired consistently into *both* the earnings basis and the net-debt bridge (the §11 lease-basis check). A liability sitting on the balance sheet but absent from the bridge, with post-AASB 16 earnings in the numerator, is the mixed-basis failure this convention exists to catch.
