...
**Version:** 0.8
**Date:** 21 April 2026 (drafted); 7 May 2026 (Ben's-bot review + Group 1 changes; editorial sweep; v0.1 freeze); 17 May 2026 (v0.2: archetype-granularity principle added at §7.1.1); 22 May 2026 (v0.2.1: IPL → DNL rename editorial sweep); 25 May 2026 (v0.3: equity-bridge and valuation-mechanics methodology added); 28 May 2026 (v0.3.1: single-WACC discipline resolved); 9 June 2026 (v0.4: Five-Forces spine for company position §3.3, tax-rate discipline §3.6, source-document ingestion §14); 12 June 2026 (v0.5: bank-specific valuation conventions methodology §15 added); 17 June 2026 (v0.6: six discipline / explicit-articulation refinements from DNL & WBC review); 25 June 2026 (v0.7: six process-discipline refinements surfaced by the CSL build — §3.7, §16.5, §11.6, §4.4.1, §3.5.7, §14.5.2); 12 July 2026 (v0.8: lease-normalisation convention added at methodology §4.6).
**Status:** v0.8. Section-by-section review with Tara complete; Ben's-bot platform-side review (5 May 2026) incorporated; archetype-granularity principle added 17 May 2026; test-company name change IPL → DNL swept through the spec 22 May 2026; equity-bridge methodology added 25 May 2026. Bank-specific methodology fork added 12 June 2026 ahead of WBC build. v0.6 (17 June 2026) consolidated six discipline refinements from DNL/WBC review; v0.7 (25 June 2026) added six process-discipline refinements surfaced by the CSL build; v0.8 (12 July 2026) added the lease-normalisation convention. Subsequent material changes will bump version with a migration note.

**Migration from v0.7 → v0.8:** Substantive methodology addition, additive only — no schema breaks; existing company YAMLs continue to validate.

(a) **§4.6 Lease-normalisation convention (AASB 16 / IFRS 16).** New subsection of the equity-bridge methodology (`design/methodology/equity_bridge_and_valuation_mechanics.md` §4.6). Codifies a single convention that must hold across the three places leases touch the number — the operating build, the net-debt / equity bridge, and the multiples cross-check. Default treatment is **Approach A** (leases as debt: reported post-AASB 16 earnings plus the lease liability in net debt), governed by a subject-consistency rule (§4.6.2) and a two-ratio materiality gate shown on screen (§4.6.3). Establishes that cross-company lease distortion is a *multiples* problem, not a DCF problem (§4.6.4 — in the subject's own DCF the tenure choice offsets at the equity line), addressed by a house re-capitalisation to a lease-neutral **EV/EBITDAR** peer basis (§4.6.5, three fixed knobs) that also neutralises the AASB 16 vs US GAAP (ASC 842) EBITDA/OCF divergence (§4.6.6). The EV/EBITDAR basis is **gated to lease-heavy archetypes** — the three current test companies (DNL, WBC, CSL) do not invoke it.

(b) **DNL worked example (§4.6.7).** DNL is lease-light (liability ~2.6% of EV; lease cost ~6–7% of EBITDA, both below the gate) but was in a latent mixed basis — post-AASB 16 earnings with a net-debt definition that excluded the AUD 212m lease liability. Folding the liability into net debt is a ~AUD 0.11 / ~3% consistency correction (not a materiality judgment), re-anchoring the six scenarios (Muddle Through 3.59 → 3.48) with the downside/upside asymmetry ratio unchanged.

(c) **Data contract (§4.6.8).** Peer re-capitalisation needs four fields per company beyond the reported liability: whether total debt already includes lease liabilities, the annual rent / lease-cost line, the undiscounted lease-maturity table, and an accounting-standard flag. Carried as a `lease_contract` block on company-position data (mock `_leaseContract` stub on the DNL config until the EODHD feed returns). The §11 workbook lint gains a lease-basis consistency check (a liability on the balance sheet must be wired into both the earnings basis and the net-debt bridge).

Cross-cutting principle observed (as with v0.6 / v0.7): the convention is a discipline / consistency refinement. The only per-share movement is the DNL lease-basis correction the convention surfaced (Muddle Through 3.59 → 3.48, owner-accepted), not a change in analytical substance.
...
**Migration from v0.6 → v0.7:** Process-discipline refinements surfaced by the CSL build (25 June 2026). Additive only — no schema breaks; existing company YAMLs continue to validate. Reference: `design/reviews/methodology_v0_7_proposal_2026-06-25.md` (accepted 25 June 2026).

(a) **§3.7 Forecast-trajectory discipline (cycle-position split).** Every primary growth and margin driver is entered as a trajectory stating (i) current cycle position, (ii) any live, high-certainty disruption, and (iii) the through-cycle anchor it converges to. A flat CAGR must be justified, not assumed as the default input shape. Surfaced by CSL Behring (entered as a flat 5% CAGR, blind to the live Medicare Part D + China trough the archetype itself rates high-certainty at 1H26 Behring −7%); fixed via a J-curve (FY26 −1%, recovering to 5.5% by FY29). Sister to the DNL gas-roll-off overlay §3.2.1; cross-references the §11.3 time-profile library. Additive schema: a per-driver trajectory block on company YAMLs.

(b) **§16.5 External reference forecast as a calibration input.** Pull a consensus (or strategist) explicit-period forecast at build time and sit it beside the model *during* construction, articulating per year and per driver where and why the framework differs. Per §16 the gap remains the output — the framework is not tuned to consensus — but the divergence must be known and explained while building. Had the −1.1% FY26 consensus revenue been in front of us during the CSL build, the missing Behring J-curve would have been obvious immediately. Process/discipline; no schema change.

(c) **§11.6 Workbook-integrity checks (orphan-input + terminal-continuity).** Two automated checks, implemented as a reusable lint (`scripts/workbook_lint.py`): (1) no orphan inputs — every yellow/blue Assumptions input cell must be referenced by at least one formula; structural orphans (terminal, discount-rate, growth, margin drivers) are errors, documentation-only anchors (measured beta shown for transparency, FY25 memos, reconciliation checks) are warnings and should carry a memo marker in their label/note; (2) terminal continuity — the terminal block must be driven by the stated structural assumptions, and the terminal EBIT margin must not exceed the explicit-period exit-year margin, else a written justification is required. Surfaced by the CSL v1 terminal-margin orphan (stated 30% but never wired; the model silently capitalised FY31's ~33.3% peak into perpetuity — ~84% of the USD 23/share correction once fixed). The lint is palette-aware (recognises both the CSL/WBC `FFF2CC`/`0066CC` and the DNL `FFFF00`/`0000FF` input colour conventions). Backported to DNL (v6) and WBC (v4); both lint-clean.

(d) **§4.4.1 Benefit-funding consistency (extends the §4.4 cost-of-closure rule).** Any modelled benefit (margin uplift, synergy) is checked against whether its enabling spend is *also* generating modelled growth; if transformation savings are reinvested to drive growth, the full savings cannot also be banked as margin. The reinvestment split becomes an explicit input rather than being assumed away. Surfaced by CSL banking the full peer-gap-closure margin uplift *and* the growth the reinvested US$525m is meant to fund (a classic double-count); the v3 reforecast cut the Muddle Through Behring uplift to +150bps with the full +250bps reserved for Orderly Convergence. Additive schema: a reinvestment-split input.

(e) **§3.5.7 Market-implied cost-of-capital reality check.** The selected cost of equity — and, by extension, the terminal margin and growth — is sanity-checked against the market-implied figures via a single-lever reverse-DCF, especially after a large drawdown. Surfaced by CSL's β 0.85 (defensive-healthcare peer triangulation) sitting against a 60%+ share-price fall; the cross-check showed the market implies a cost of equity near 13% (β ≈ 1.7), or a terminal margin near half the FY25 level, or perpetual contraction — none plausible standing alone, so the residual premium is genuine framework-vs-market disagreement per §16, not a calibration error. β held at 0.85 by owner decision (repeatable framework, no intervention). Reporting diagnostic only — the framework is **not** re-tuned to the implied figure.

(f) **§14.5.2 Aggression flag (extends §14.5.1 assumption-strength tagging).** Where a `[judgment]` input sits *above* disclosed company guidance or the archetype secular rate, it is surfaced explicitly — these are the model's boldest, least-supported bets. Surfaced by CSL's +250bps Behring margin uplift and flat 5% growth both sitting above near-term guidance at a moment the street was downgrading. Additive: an aggression tag alongside the §14.5.1 strength tags.

Cross-cutting principle observed (as with v0.6): **all six are discipline / explicit-articulation refinements, not analytical-substance changes.** The only per-share movement is the CSL reforecast the discipline surfaced (Muddle Through USD 164.97 → 134.52 across v1→v4, owner-accepted), not a change in methodology substance.
...
**Migration from v0.3.1 → v0.4:** Substantive methodology additions, additive only — no schema breaks.

(a) **§3.3 Company position via Five Forces (spine).** The Step 3 company-position translation is structured force-by-force, mirroring the §7 industry-archetype Five Forces analysis. Each company-position offset is anchored to a specific force with a named mechanism, replacing the earlier catch-all "company offset." See methodology §3.3 for the structure and DNL worked example.

(b) **§3.6 Tax-rate discipline.** Effective rate (per management guidance) glides to blended statutory (computed from jurisdictional weights × statutory rates) over the explicit horizon. Single rate principle: same blended statutory used for terminal operating tax, WACC debt-tax-shield, and Hamada re-levering of beta. Internal consistency.

(c) **§14 Source-document ingestion.** Standing checklist of source documents per company (investor presentations, statutory reports, continuous disclosure, transcripts, sustainability). Per-company `documents` register. Schema validator requires explicit reference to corporate-action documents when overlay is active. Refresh cadence aligned to half-year reporting. Handoff spec for Ben's data workstream.

Schema-level implications listed in §3.3 of the methodology document. All v0.3.1 schemas remain valid (additive only).

**Status:** The parked §9.7 review item 4 ("WACC scenario behaviour") is resolved in favour of holding WACC constant across scenarios. Scenario rate-driver deltas no longer flow into the DCF discount rate. Terminal growth remains scenario-conditional. See `design/methodology/equity_bridge_and_valuation_mechanics.md` §3.5. Spec change is additive only (resolution of a parked decision); no schema or analytical content changes.
